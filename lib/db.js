...
/** Tổng PnL của mọi lệnh ĐÃ ĐÓNG — để dựng lại vốn thay vì đặt về $200. */
async function tongPnlDaDong() {
  const r = await truyVan('SELECT COALESCE(SUM(pnl_usd),0) t FROM lenh WHERE ts_dong IS NOT NULL');
