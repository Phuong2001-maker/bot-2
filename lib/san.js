...
/* ⛔ NHÃN SỰ KIỆN LÀ `SAN_OKX`, KHÔNG PHẢI `SAN`.
   Máy trạng thái đã có sẵn trạng thái `TT.SAN` nghĩa là "sẵn sàng, đứng
   ngoài". Ghi sự kiện sàn với nhãn `SAN` thì hai thứ hoàn toàn khác nhau
   dồn chung một chỗ và không tách được khi đọc DB — đã dính đúng lỗi này
   ngay lần truy vấn đầu tiên. */
...
async function datSL(L) {
  ...
  if (!q) {
    ...
    DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_HONG', lyDo: 'khong_quy_doi_duoc',
  ...
  if (!laThat() || !_sanSang) {
    ...
    L.slAlgoId = null;
    DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_KHO', lyDo: 'se_dat_SL',
  ...
  if (!kq.ok) {
    ...
    DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_HONG', lyDo: 'dat_that_bai',
  ...
  DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_DAT', lyDo: 'ok',
...
async function suaSL(L) {
  ...
  if (!laThat() || !_sanSang) {
    DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_KHO', lyDo: 'se_doi_SL',
  ...
  DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_DOI', lyDo: 'ok',
...
async function huySL(L) {
  if (!laThat() || !_sanSang) {
    if (L.slGia) DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_KHO', lyDo: 'se_huy_SL',
                                chiTiet: { gia: L.slGia } });
    return true;
  }
  ...
  if (!kq.ok) {
    ...
    DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_MO_COI', lyDo: 'huy_that_bai',
                   chiTiet: { algoId: L.slAlgoId, code: kq.code } });
    return false;
  }
  DB.ghiSuKien({ coin: L.sym, tu: 'SAN_OKX', den: 'SL_HUY', lyDo: 'ok', chiTiet: { algoId: L.slAlgoId } });
  return true;
}
