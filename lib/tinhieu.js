...
/* Hash tự sinh từ chính mảng tham số quyết định → không bao giờ quên
   ...
   ⛔ SỬA 2026-08-24: bản cũ CHỈ băm trọng số tín hiệu và ngưỡng thoát —
   KHÔNG hề có nhóm `SETUP`. Nên khi hạ ngưỡng SHORT-A từ 30%/0,05% xuống
   20%/0,02%, băm ĐỨNG IM và DB không phân biệt được hai đợt dữ liệu.
   Đúng cái mà chú thích này tự nhận là sẽ không bao giờ xảy ra.

   Nay băm cả điều kiện vào lệnh (`SETUP`), bộ lọc coin (`LOC_COIN`),
   ngân sách rủi ro, vốn và các trần. Có phép kiểm HÀNH VI bắt băm phải
   đổi khi ngưỡng setup đổi — không tin vào chú thích nữa. */
...
function verTrongSo(cfg) {
  const v = [
    ...
    /* ⭐ điều kiện VÀO LỆNH — thứ quan trọng nhất mà bản cũ bỏ sót */
    JSON.stringify(cfg.SETUP || {}),
    JSON.stringify(cfg.LOC_COIN || {}),
    JSON.stringify(cfg.TRAN_RUI_RO || {}),
    cfg.VON, cfg.SO_LENH_MO_TOI_DA, cfg.SO_COIN_TRAN,
    cfg.QUET ? cfg.QUET.MOC_PUMP_LON : null,
  ].join('|');
