...
class QuanLyLenh {
  ...
  /** Rủi ro còn lại tách theo hướng — chặn TẬP TRUNG TƯƠNG QUAN.
   *  20 lệnh long trên 20 alt lúc BTC sập không phải 20 lệnh, đó là 1 lệnh
   *  cỡ 20× — trần rủi ro TỔNG không bắt được điều đó. Đo thật trên 56
   *  lệnh đầu: 53 lệnh là LONG, tức danh mục chỉ là MỘT vị thế.
   *  ⛔ Từ 2026-08-24 đây là CỔNG THẬT (`TRAN_CUNG_HUONG_PC`), không còn
   *  chỉ để cảnh báo — đổi cùng lượt mở khoá SHORT. */
  ruiRoTheoHuong() {
  ...
  /* =================================================================== */
  /*  ĐỐI SOÁT LÚC KHỞI ĐỘNG — dựng lại lệnh đang mở từ DB               */
  /* =================================================================== */
  /**
   * ⛔ VÁ LỖI: trước 2026-08-24 trạng thái lệnh CHỈ nằm trong RAM. Bot
   * chết vì bất cứ lý do gì — OOM, reboot, crash — thì cron bật lại sau
   * ≤60 giây nhưng lệnh đang mở BIẾN MẤT khỏi bộ nhớ: không ai canh nữa,
   * và bản ghi kẹt vĩnh viễn trong DB với `ts_dong = NULL`.
   * Ở chế độ giấy chỉ mất mẫu; ở tiền thật đó là một vị thế THẬT trên sàn
   * mà bot không biết nó tồn tại.
   *
   * ⭐ Khôi phục được từ CỘT ĐÃ CÓ, không cần thêm cột nào:
   *   · `gia_cat` được ghi mỗi 2 giây và đường cắt CHỈ SIẾT VÀO, nên nạp
   *     lại mốc chặt nhất từng đạt là chính xác tuyệt đối — không có
   *     chuyện phục hồi xong đường cắt lại rộng ra.
   *   · `giaDinh` suy ngược từ `gia_cat` và `khoang_trailing`.
   *   · `daKhoaVon` suy từ việc `gia_cat` đã vượt điểm hoà vốn chưa.
   *
   * ⚠ `baoDong` KHÔNG khôi phục được (nó chỉ có trong RAM) → đặt `false`.
   * Phía an toàn: lệnh cần cò đảo chiều nổ LẠI mới chốt lời, chứ không
   * chốt nhầm ngay sau khi khởi động.
   */
  napLaiLenhMo(dong) {
    let n = 0;
    for (const r of dong || []) {
      ...
      if (st.lenh) continue;                       // đã có rồi thì không đè

      const giaVaoTB = parseFloat(r.gia_vao_tb);
      const giaCat = parseFloat(r.gia_cat);
      const notional = Number(r.gia_tri_lenh_usd);
      const K = Number(r.khoang_trailing) || this.cfg.TRAILING.KHOANG_SAN;
      if (!(giaVaoTB > 0) || !(giaCat > 0) || !(notional > 0)) {
        canh(`[${sym}] ⛔ ban ghi lenh mo THIEU DU LIEU (id ${r.id}) — bo qua`);
        continue;
      }
      const huong = r.huong === 'short' ? 'short' : 'long';
      /* đỉnh giá suy ngược từ đường cắt: giaCat = giaDinh × (1∓K) */
      const giaDinh = huong === 'short' ? giaCat / (1 + K) : giaCat / (1 - K);
      const hoaVon = this._giaHoaVon(huong, giaVaoTB);
      const daKhoaVon = huong === 'short' ? giaCat <= hoaVon : giaCat >= hoaVon;
      const dinhLai = huong === 'short'
        ? (giaVaoTB - giaDinh) / giaVaoTB : (giaDinh - giaVaoTB) / giaVaoTB;

      const L = {
        id: r.id, uid: r.uid, sym, huong, setup: r.setup,
        tsMo: Number(r.ts_mo), gioVN: r.gio_vn_mo, trongKhung: !!r.trong_khung,
        lanVao: [], soCoin: notional / giaVaoTB, notional,
        kyQuy: Number(r.ky_quy_usd) || 0, soLanDCA: Number(r.so_lan_dca) || 0,
        giaVao1: parseFloat(r.gia_vao_1) || giaVaoTB, giaVaoTB,
        khoangTrailing: K,
        loThietKeUsd: Number(r.lo_thiet_ke_usd) || K * notional,
        giaDinh, giaCat, daKhoaVon,
        dinhLai: Math.max(0, dinhLai), baoDong: false, hoiLai: 0, nguongHoi: 0,
        pnlUsd: 0, pnlThucHien: 0, pnlPcGia: 0,
        fundingNhanUsd: Number(r.funding_nhan_usd) || 0,
        phiUsd: Number(r.phi_usd) || 0, truotUsd: Number(r.truot_usd) || 0,
        soLanChot: Number(r.so_lan_chot) || 0, canhBao: [], raoChan: null,
        slAlgoId: r.sl_algo_id || null,
        slGia: r.sl_gia ? parseFloat(r.sl_gia) : null,
      };
      st.lenh = L;
      st.huong = huong;
      st.setup = r.setup;
      st.trangThai = r.trang_thai && BAC[r.trang_thai] >= BAC.THAM_DO ? r.trang_thai : TT.THAM_DO;
      n++;
      ghi(`[${sym}] ♻ NAP LAI lenh dang mo · ${huong.toUpperCase()} ${r.setup}`
        + ` · vao ${giaVaoTB} · cat ${giaCat}${daKhoaVon ? ' 🔒' : ''}`
        + ` · mo tu ${new Date(Number(r.ts_mo)).toISOString().slice(0, 16).replace('T', ' ')}`);
      DB.ghiSuKien({ coin: sym, tu: 'KHOI_DONG', den: st.trangThai, lyDo: 'nap_lai_lenh_mo',
                     chiTiet: { id: r.id, giaCat, daKhoaVon, khoangTrailing: K } });
    }
    if (n) ghi(`♻ da nap lai ${n} lenh dang mo tu DB`);
    return n;
  }

  /** Dựng lại vốn từ tổng PnL đã đóng, thay vì đặt về VON mỗi lần restart.
   *  Trước đây con số vốn trên log là của PHIÊN, không phải cộng dồn. */
  napLaiVon(tongPnl) {
    if (!isFinite(tongPnl)) return;
    this.von = this.vonBanDau + tongPnl;
    this.vonThapNhat = Math.min(this.vonThapNhat, this.von);
    ghi(`♻ von dung lai tu DB: $${this.von.toFixed(2)} (goc $${this.vonBanDau} + PnL da dong $${tongPnl.toFixed(2)})`);
  }
  ...
  _capNhatDuongCat(st, gia, x) {
    ...
    /* Đường cắt vừa siết → đẩy mốc mới sang sàn. `San.suaSL` tự bỏ qua
       nếu chưa vượt VÙNG CHẾT, nên gọi mỗi nhịp cũng không spam API. */
    if (L.giaCat !== catCu) {
      San.suaSL(L)
        .then(doi => { if (doi) DB.capNhatSL(L.id, L.slAlgoId, L.slGia); })
        .catch(e => canh(`[${st.sym}] doi SL loi:`, e.message));
    }
  ...
  _moLenh(st, P, A, E, boiCanh) {
    ...
    /* ⭐ TRẦN THEO HƯỚNG — chặn danh mục dồn hết về một phía.
       Trần TỔNG ở trên không bắt được tương quan: N lệnh long trên N alt
       lúc BTC sập không phải N lệnh, đó là 1 lệnh cỡ N×. Đo thật: 53/56
       lệnh đầu là LONG, tức danh mục thực chất chỉ là MỘT vị thế. */
    const rrHuong = this.ruiRoTheoHuong()[st.huong] || 0;
    const tranHuong = cfg.TRAN_RUI_RO.TRAN_CUNG_HUONG_PC * tran;
    if (rrHuong + ruiRoMoi > tranHuong)
      return chan('chan_tran_cung_huong', {
        huong: st.huong, dangCo: +rrHuong.toFixed(2),
        them: +ruiRoMoi.toFixed(2), tran: +tranHuong.toFixed(2),
      });
    ...
    /* ⭐ ĐẶT SL PHÍA SÀN — Giai đoạn 10.
       ⛔ KHÔNG chờ kết quả: đây là vòng lặp 2 giây, một lời gọi mạng chậm
       sẽ kéo lùi việc quản lý MỌI coin khác. Đường cắt trong RAM vẫn canh
       đồng thời, nên SL sàn là lớp CỘNG THÊM chứ không phải lớp thay thế —
       hỏng nó thì mất lớp nhanh, không mất hết lưới. */
    San.datSL(L)
      .then(kq => { if (kq) DB.capNhatSL(L.id, L.slAlgoId, L.slGia); })
      .catch(e => canh(`[${st.sym}] dat SL loi:`, e.message));
  ...
  _dong(st, gia, lyDo, E, A) {
    ...
    /* ⛔ HUỶ SL TRƯỚC TIÊN. Quên huỷ = SL mồ côi nằm lại trên sàn, và lần
       sau vào lệnh cùng coin nó có thể kích hoạt cắt nhầm vị thế mới. */
    San.huySL(L).catch(e => canh(`[${st.sym}] huy SL loi:`, e.message));
