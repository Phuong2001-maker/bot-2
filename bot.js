...
async function quet() {
  try {
    ...
    /* Vùng đệm theo THỨ HẠNG — lớp trễ thứ hai, chỉ có nghĩa khi trần bị
       chạm: coin đang theo dõi mà tụt xuống ngay dưới trần thì chưa gỡ. */
    const giu = new Set(dat.slice(0, cfg.SO_COIN_TRAN + cfg.QUET.VUNG_DEM_COIN).map(t => t.sym));
    let daGo = 0;
    for (const sym of [...engines.keys()]) {
      ...
      if (!giu.has(sym) && daGo < cfg.QUET.GO_TOI_DA_MOI_VONG) { dungEngine(sym); daGo++; }
    }
    for (const sym of ghim) if (!engines.has(sym)) taoEngine(sym);
    for (const t of dat) {
      if (engines.size >= cfg.SO_COIN_TRAN) break;
...
function nhipPhanTich() {
  ...
  for (const E of engines.values()) {
    try {
      /* nhịp tim THẬT là gói DỮ LIỆU, không phải pong */
      if (ws.tuoiData('books', E.instId) > cfg.QUET.SO_LENH_HET_HAN_MS) continue;
      if (E.so.hong) continue;

      const A0 = E.so.lay(E.ctVal);
      if (!A0.bids.length || !A0.asks.length) continue;
      ...
      if (Math.abs(P.S) >= cfg.NGUONG_GHI && amMay >= cfg.WARMUP_GIAY) {
        ...
        if (co && now - lanCuoi > cfg.NGUOI_LANH_PHUT * 60e3) {
          ...
          const giaCat = QL._giaTaiMucLo(huongTH, P.mid, notional0, cfg.CAT_LO_USD);
  ...
  for (const E of engines.values()) {
    ...
    if (st.lenh) {
      ...
      lenhMo.push({
        ...
        catLoUsd: cfg.CAT_LO_USD, catLoTran: cfg.CAT_LO_USD_TRAN,
        dongBang: true, diem,
      });
    } else {
      theoDoi.push({
        ...
        keHoach: st.keHoach ? {
          ...
          notional: st.keHoach.notional, catLoUsd: st.keHoach.catLoUsd,
...
function inSucKhoe() {
  ...
  ghi(`${engines.size} coin · loc=${soQuaLoc}/${cfg.SO_COIN_TRAN}`
