...
|---|---|---|
| **Có ĐỘNG không** | `(high24 − low24) / last ≥ 0,08` | coin đi ngang thì loại — coin biến động 2%/ngày không setup nào sống nổi. ⭐ Từ v2.0 biên độ 24h còn là **thước định cỡ đường cắt** (`K = 0,50 × biên độ`), nên ngưỡng này ảnh hưởng cả rủi ro chứ không chỉ việc chọn coin |
...
⛔ **Hai số này KHÔNG liên quan nhau và không được nới cùng nhau.** Coin theo dõi là cần gạt lấy mẫu (nới thoải mái, chỉ tốn hạ tầng). Trần lệnh mở là cần gạt rủi ro: **lỗ đồng thời tối đa = tổng RỦI RO CÒN LẠI của các lệnh đang mở** — từ v2.2 đây chính là cổng chặn, thay cho trần đếm N. Ngân sách 30% vốn giữ nguyên như thời N=3 × $20, nhưng vì rủi ro mỗi lệnh giảm còn $1,80–$4,80 nên nó mua được 12–33 lệnh. Lệnh đã khoá hoà vốn chiếm 0 ngân sách. Thêm nữa, 10 lệnh short trên 10 alt lúc BTC bật **không phải 10 lệnh mà là 1 lệnh cỡ 10×** — và cũng chỉ đáng ~1–2 lệnh độc lập khi đếm cỡ mẫu, nên nới nó còn làm **hỏng phép đo** chứ không chỉ tăng rủi ro.
...
| 2026-08-23 | **v2.0 — BỎ CẮT LỖ CỐ ĐỊNH, thay bằng ĐƯỜNG TRAILING BÁM ĐỈNH** | Chủ dự án chốt sau khi đọc dump 11,3 ngày đầu (**54 lệnh**, 311.513 nhịp, 87 coin). Ba con số buộc phải đổi: (1) **tỷ lệ mạo hiểm 1:50** — cắt lỗ $20 trên notional $60 = giá phải chạy **33%**, trong khi chốt lời nhả ra ở **~0,64%** giá (đỉnh trung vị 1,288%, công thức cũ trả lại đúng nửa) → phải thắng **~96%** mới hoà vốn, thực tế thắng **64,8%**; (2) toàn bộ khoản lỗ **−$15,49** đến từ **đúng 2 lệnh BEAT** chạm mốc $20 (−$20,80 và −$20,78), còn 52 lệnh kia cộng lại **+$26,10**; (3) tổng đỉnh lãi **+$71,56** nhưng về đích **−$15,49** → **đã trả lại thị trường $87,05**. **Cơ chế mới:** một đường cắt duy nhất cách **đỉnh giá đã đạt** một khoảng `K = kẹp(0,50 × biên độ 24h của coin, 3%, 8%)`, **chỉ đi một chiều — không bao giờ lùi ra** (cưỡng chế bằng `_dayGiaCat()` + phép kiểm quét mã nguồn), cộng **khoá hoà vốn** khi lãi đạt 1×K — chính ca BEAT #2 từng lãi **+1,34%** rồi quay về −$20,78 mà không có gì chặn. Van cuối `LO_TRAN_USD = $12` **chỉ** để chặn giá **nhảy qua** đường cắt, không phải cơ chế cắt. **Sàn/trần chọn bằng dữ liệu, không đoán:** dựng lại đường giá theo phút của 53/54 lệnh rồi đo độ sâu đi ngược tối đa — cắt ở 2% giết oan 13 lệnh lãi (+$4,83), ở 4% giết oan 5 lệnh (+$0,95), ở 6% chỉ giết oan 3 lệnh (+$0,73) mà vẫn chặn được cả hai lệnh BEAT (đi ngược −31,9% và −28,1%). **Chốt lời giữ cơ chế cò**, siết hai chỗ: `HOI_LAI_TRAN` **0,50 → 0,35** (47/54 lệnh có đỉnh <10% nên luôn rơi vào nhánh này — giữ 65% thay vì 50%) và thêm **SÀN PHÍ** `SAN_CHOT_LOI_PC = 0,24%` = 2× phí khứ hồi (17 lệnh đóng bằng `chot_loi` mà PnL **âm**; 7 lệnh có đỉnh dưới sàn → chốt ở đó **không thể** dương, là toán học chứ không phải xui). ⚠ **Hai hệ quả dây chuyền suýt lọt:** cửa sổ DCA `[$4,$10]` nằm **ngoài tầm với** khi lỗ tối đa chỉ còn $1,80–$4,80 → đổi sang **tỷ lệ** `[0,20 ; 0,50]` × rủi ro thiết kế, nếu không DCA **chết âm thầm không lỗi không log**; mẫu số **R** đổi từ hằng số $20 sang `lo_thiet_ke_usd` của từng lệnh, nếu không R của coin êm và coin loạn không so được. `RAO_CHAN_TOI_THIEU` giữ 45 phút và **vẫn đo tới van cuối** chứ không tới đường trailing — đo tới đường trailing thì số phút tụt hơn chục lần và DCA chết lần nữa; đổi lại DCA sẽ nổ **thưa hơn**, **chưa hiệu chỉnh lại bằng dữ liệu**. Thêm 2 cột `khoang_trailing` · `lo_thiet_ke_usd` vào bảng `lenh`. **226/226 đạt** (trước 205) |

| 2026-08-23 | **v2.1 — vá lỗ hổng "sổ chết thì lệnh đang mở không ai canh"** | Đi kèm bắt buộc với v2.0. `bot.js` có **ba lệnh `continue`** nằm trước `QL.capNhat()` (sổ quá 20s không có gói · `E.so.hong` · sổ rỗng một phía) → khi sổ chết, lệnh đang mở **không được kiểm đường cắt lẫn van cuối**. Lỗ hổng có từ trước nhưng được **đường cắt 33% che cho**; đường cắt mới 3–8% làm tấm che biến mất. **Đo thật trong log 11 ngày:** 19/08 01:35→01:40 **6 phút, 20/20 coin mất sổ, WS im, 3 lệnh đang mở** — vốn và bộ đếm ghi DB đứng nguyên suốt, chứng minh `capNhat()` không chạy; tổng 8 phút "0 sổ live" (6 phút có lệnh mở); thêm 10.333 lần `so lenh hong` từ đồng bộ định kỳ 30 phút. **Vá:** `canhLenhMu()` canh lệnh bằng **giá REST ticker** — đường dữ liệu độc lập với WebSocket. Chỉ chạy hai đường ra bảo vệ vốn; không chốt lời (cò cần sổ), không mở lệnh mới, không DCA. Ticker quá 3 phút → `khong_co_gia`, kêu to chứ không đoán. Đóng mù phạt trượt 1%. Logic đường cắt tách thành `_capNhatDuongCat()` dùng chung hai nhánh, có phép kiểm chặn việc chép ra hai bản. ⛔ Nguyên tắc mới: **sổ hỏng chặn MỞ lệnh, không bao giờ chặn ĐÓNG lệnh.** **244/244 đạt** |

| 2026-08-23 | **v2.2 — bỏ trần ĐẾM ở cả hai cần gạt, thay bằng RỦI RO và RAM** | Chủ dự án chốt: trần hiện tại **cắt mất cơ hội có lãi**. Đo thật: trần 3 lệnh khoá bot **29,3% thời gian**; **37/71** lần SẴN_SÀNG không bao giờ mở được lệnh. **Lệnh mở** `SO_LENH_MO_TOI_DA` 3 → `null`, thay bằng ngân sách rủi ro **30% vốn** = đúng ngân sách cũ (3 × $20 / $200); vì rủi ro mỗi lệnh giảm từ $20 xuống $1,80–$4,80 nên cùng ngân sách đó mua được **12–33 lệnh**. Ngân sách đo bằng **rủi ro CÒN LẠI** = PnL tại đường cắt → lệnh **đã khoá hoà vốn chiếm 0 chỗ**, trả lại ngân sách cho lệnh mới. ⚠ Trần rủi ro **không** bắt được tương quan (53/54 lệnh LONG) → thêm cảnh báo `TAP TRUNG`, chỉ kêu không chặn. **Coin theo dõi** `SO_COIN_TRAN` 20 → `null`, thay bằng `RAM_TRAN_MB`; vượt `RAM_XA_BOT_MB` thì gỡ bớt coin chứ không chỉ ngừng thêm — bot OOM kill là không ai canh lệnh đang mở. Coin có lệnh mở miễn nhiễm khỏi việc gỡ. Kèm: **mọi lối chặn mở lệnh đều ghi sự kiện**, trước đây hai lối là `return` trần nên DB mù. **261/261 đạt** |

| 2026-08-24 | **v2.3 — mở khoá SHORT, thêm trần theo hướng, gỡ số cứng 0.30** | Chủ dự án chốt bot phải có lệnh short mà vẫn an toàn. Đo **311.513 nhịp** tìm chỗ nghẽn thật: `chg24 ≥ 30%` đạt 4,25% số nhịp và `funding ≥ 0,05%` đạt 4,94% — **cả hai đều không hiếm**, nhưng **giao chỉ 0,22%**, qua cả 4 vế còn **395 nhịp trên 3 coin** trong 11,3 ngày → đúng 1 lệnh short. Hai vế gần như loại trừ nhau (pump mạnh thì funding thấp, funding cao thì không pump). Sâu hơn: **41% nhịp có funding 0,01–0,02% = mức nền OKX**, nên 0,05% là 5 lần mức nền. Hạ `SHORT-A` xuống **20% / 0,02%** → 1.659 nhịp (**4,2×**), vẫn 5 coin. ⛔ Không hạ funding tới 0,01% dù cho 20,8×: đó là mức nền, tới đó vế funding ngừng lọc và giả thiết "long chen chúc" tan. Giữ nguyên 100% chốt chặn squeeze; SHORT vẫn khắt khe hơn LONG-A. Kèm **trần theo hướng** `TRAN_CUNG_HUONG_PC = 0,70` (cảnh báo cũ nâng thành cổng) vì trần rủi ro tổng không bắt được tương quan — 53/56 lệnh đầu là LONG. Và gỡ **số cứng `0.30`** trong `bot.js` điều khiển `da_tang30_hom_qua` — cổng hai chiều của SHORT-B và LONG-B — thành `QUET.MOC_PUMP_LON`. **271/271 đạt** |

| 2026-08-24 | **v3.0 — GIAI ĐOẠN 10: cắt lỗ phía sàn, đối soát khởi động (khoá ở chế độ giấy)** | Chủ dự án chỉ ra điểm yếu thật: lúc biến động mạnh, đường cắt trong RAM có **ba tầng trễ** — nhịp 2 giây (và trailing 3–8% nhạy cảm hơn hẳn cắt $20 = 33% giá), Node đơn luồng (20 lệnh cùng chạm cắt thì lệnh cuối chờ lệnh đầu), và bot chết thì không ai cắt. Giải: đưa SL sang **máy khớp lệnh của sàn** — canh micro-giây, song song, sống độc lập với bot. **BA CHẾ ĐỘ MỘT ĐƯỜNG CODE:** `giay` (mặc định, không cần khoá, vẫn tính đủ lệnh SL và ghi `SL_KHO` để soi) → `demo` (API thật + `x-simulated-trading`, tiền ảo) → `that` (bỏ đúng header đó). Thêm `lib/okx-tt.js` (ký HMAC, endpoint riêng tư) và `lib/san.js` (bộ chuyển). SL kiểu **thị trường** `slOrdPx:'-1'` + `reduceOnly` — SL giới hạn có thể không khớp, mà SL không khớp còn tệ hơn không có SL. **Vùng chết 0,3%** chống spam sửa lệnh; gọi sàn **không `await`** trong vòng lặp 2 giây. Vá luôn lỗi mất trạng thái: `napLaiLenhMo()` dựng lại lệnh đang mở từ DB (đường cắt phục hồi đúng mốc chặt nhất vì `gia_cat` chỉ siết vào), `napLaiVon()` dựng vốn từ PnL đã đóng. Đối soát chạy **trước** khi bật nhịp, và định kỳ 5 phút ở chế độ thật — chỉ cảnh báo, **không tự đóng sổ sách** bằng giá đoán. Thêm cột `sl_algo_id` · `sl_gia`. **304/304 đạt** |
